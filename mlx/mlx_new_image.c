...
void	*mlx_int_new_xshm_image(t_xvar *xvar,int width,int length ,int format)
{
  t_img	*img;
  int	(*save_handler)();
  ...
  img->image = XShmCreateImage(xvar->display,xvar->visual,xvar->depth,
			       format,img->data,&(img->shm),width,length );
  if (!img->image)
    {
      free(img);
      return ((void *)0);
  ...
  img->format = format;
  img->shm.shmid = shmget(IPC_PRIVATE,(width+32)*length *4,IPC_CREAT|0777);
  if (img->shm.shmid==-1)
    {
...
void	*mlx_int_new_image(t_xvar *xvar,int width, int length ,int format)
{
  t_img	*img;

  if (!(img = malloc(sizeof(*img))))
    return ((void *)0);
  if (!(img->data = malloc((width+32)*length *4)))
  {
    free(img);
    return ((void *)0);
  }
  bzero(img->data,(width+32)*length *4);
  img->image = XCreateImage(xvar->display,xvar->visual,xvar->depth,format,0,
			    img->data,width,length ,32,0);
  if (!img->image)
    {
      free(img->data);
      free(img);
  ...
  img->width = width;
  img->length  = length ;
  img->pix = XCreatePixmap(xvar->display,xvar->root,width,length ,xvar->depth);
  img->format = format;
...
void	*mlx_new_image(t_xvar *xvar,int width, int length )
{
  t_img	*img;

  if (xvar->use_xshm)
    if (img = mlx_int_new_xshm_image(xvar,width,length ,ZPixmap))
      return (img);
  return (mlx_int_new_image(xvar,width,length ,ZPixmap));
}

void	*mlx_new_image2(t_xvar *xvar,int width, int length )
{
  t_img	*img;

  if (xvar->use_xshm)
    if (img = mlx_int_new_xshm_image(xvar,width,length ,XYPixmap))
      return (img);
  return (mlx_int_new_image(xvar,width,length ,XYPixmap));
}
