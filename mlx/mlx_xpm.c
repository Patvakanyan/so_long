...
void	*mlx_int_parse_xpm(t_xvar *xvar,void *info,int info_size,char *(*f)())
{
		...
		if (!(line = f(info,&pos,info_size)) ||
						...
						!(length  = atoi(tab[1])) || !(nc = atoi(tab[2])) ||
						!(cpp = atoi(tab[3])) )
				RETURN;
		free(tab);
		tab = 0;
		...
		while (i--)
		{
				...
				{
						if (!(clip_data = malloc(4*width*length )) ||   ok, nice size ..
										!(clip_img = XCreateImage(xvar->display, xvar->visual,
														1, XYPixmap, 0, clip_data,
														width, length , 8, (width+7)/8)) )
								RETURN;
						memset(clip_data, 0xFF, 4*width*length );
				}
				*/
				if (method)
		...
		if (!(img = mlx_new_image(xvar,width,length )))
				RETURN;
		opp = img->bpp/8;
		...
		while (i--)
		{
				...
				while (x<width)
				{
						...
						/*
						if (col==-1)
								XPutPixel(clip_img, x, length -1-i, 0);
						else
...
void	*mlx_xpm_file_to_image(t_xvar *xvar,char *file,int *width,int *length )
{
		int	fd;
		int	size;
		char	*ptr;
...
void	*mlx_xpm_to_image(t_xvar *xvar,char **xpm_data,int *width,int *length )
{
		t_img	*img;
