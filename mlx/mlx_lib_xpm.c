...
void	*mlx_int_xpm_f_image(t_xvar *xvar, int *width, int *length,
  ...
  if (!(im2 = (void *)mlx_new_image(xvar, img1->width, img1->length))) {
    XDestroyImage(img1);
    return ((void *)0);
  }
  *width = img1->width;
  *length = img1->length;
  if (mlx_int_egal_img(im2->image, img1)) {
    bcopy(img1->data, im2->data, img1->length * img1->bytes_per_line);
    XDestroyImage(img1);
    return (im2);
  }
  if (im2->type == MLX_TYPE_SHM_PIXMAP) {
    ...
    im2->pix =
        XCreatePixmap(xvar->display, xvar->root, *width, *length, xvar->depth);
  }
...
int	mlx_int_egal_img(XImage *img1, XImage *img2) {
  if (img1->width != img2->width || img1->length != img2->length ||
...
void	*mlx_xpm_file_to_image(t_xvar *xvar, char *filename, int *width,
                            int *length) {
  return (
      mlx_int_xpm_f_image(xvar, width, length, XpmReadFileToImage, filename));
}

void	*mlx_xpm_to_image(t_xvar *xvar, char **data, int *width, int *length) {
  return (mlx_int_xpm_f_image(xvar, width, length, XpmCreateImageFromData,
                              (void *)data));
}
